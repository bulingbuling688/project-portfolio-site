...
const publishedProjects: Project[] = [
  {
    ...
    description:
      "面向跨境电商经营决策打造的一站式 AI 数据分析 Agent。只需输入业务问题，即可自动理解分析意图、检索业务知识库、生成并执行 SQL，完成销售趋势、品类表现、区域履约、客户体验等多维分析，并输出可视化图表与专业分析报告。支持连续追问、任务追踪和指标口径管理，将传统的数据查询、分析和报告流程浓缩为一次对话。",
    category: "跨境电商",
    status: "published",
    ...
    features: [
      "打造一站式跨境电商 AI 数据分析工作台，将业务提问、数据查询、经营分析和报告输出整合到一次对话中",
      "支持自然语言直接查询电商经营数据，Agent 可自动理解分析目标、规划任务、生成 SQL 并执行查询，无需手动编写代码",
      "接入完整的真实电商订单数据，覆盖订单、商品、客户、卖家、支付、评价和区域履约等核心业务对象",
      "内置销售趋势、品类表现、区域履约和客户体验等经营分析场景，帮助快速发现增长机会、异常问题和业务风险",
      "建立跨境电商业务知识库，统一 GMV、订单量、客单价、复购率和评价表现等关键指标口径，提升分析结果的可靠性",
      "支持数据库查询与知识库检索协同工作，Agent 能结合业务规则完成多表关联、指标计算和经营结论提炼",
      "提供 SQL 执行过程、数据表格、可视化图表和专业分析报告，让每一步分析过程清晰可查、结果直观可用",
      "支持连续追问、会话自动命名、历史任务回看和分析结果复用，让一次数据查询升级为可持续使用的智能经营分析流程",
    ],
    techStack: [
